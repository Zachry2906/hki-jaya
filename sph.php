...
$query = "SELECT nama, alamat, 'Dosen' as role FROM data_pribadi_dosen 
          UNION 
          SELECT nama, alamat, 'Mahasiswa' as role FROM data_pribadi_mahasiswa";
...
$pdf->Ln(10);
$pdf->Cell(20, 10, 'Dengan angka pendanaan sejumlah: ' . $pendanaan, 0, 1, 'L');
$pdf->Ln(5);
...
//penutup
// $pdf->Cell(0, 10, "PIHAK KEDUA ". date('d F Y'), 0, 1, 'R');
// $pdf->Cell(0, 10, "Sleman, ". date('d F Y'), 0, 1, 'R');
// $pdf->Ln(20);
...
// Pihak Kedua
$pdf->Cell(90, 10, '--------Nama Pencipta 2--------', 0, 0, 'L', true);
$pdf->Cell(90, 10, 'Dhimas Arief D S.T., Ph.D.', 0, 1, 'R');
$pdf->Ln(30);
$pdf->Cell(90, 10, '--------Nama Pencipta 2--------', 0, 0, 'L', true);
$pdf->Ln(40);
$pdf->Cell(90, 10, '--------Nama Pencipta 2--------', 0, 0, 'L', true);
...
//     
//     $pdf->Ln(10);
//     $pdf->Cell(90, 10, '--------Nama Pencipta 3--------', 0, 1, 'L', true);
//     $pdf->Ln(20);
...
//     $nama1 = isset($data[$i]) ? $data[$i] : "(nama)";
//     $nama2 = isset($data[$i + 1]) ? $data[$i + 1] : "(nama)";

//     $pdf->Cell(90, 10, "($nama1)", 0, 0, 'C');
//     $pdf->Cell(90, 10, "($nama2)", 0, 1, 'C');
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $nama = $row['nama']; // Ganti 'nama' dengan nama kolom yang sesuai
        $pdf->Cell(90, 10, "($nama)", 0, 0, 'C');
        $pdf->Cell(90, 10, "($nama)", 0, 1, 'C'); // Menambahkan baris baru
    }
} else {
    $pdf->Cell(0, 10, 'Tidak ada data ditemukan', 0, 1, 'C');
}
